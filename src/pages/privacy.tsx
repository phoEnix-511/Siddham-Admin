...
export default function PrivacyPolicyPage() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      .then(r => r.json())
      .then(d => {
        if (d.settings && d.settings.privacy_policy_content) {
          setContent(d.settings.privacy_policy_content);
        }
        setLoading(false);
      })
  ...
  return (
    <>
      <Head>
        <title>Privacy Policy – Siddham Wellness</title>
        <meta name="description" content="Privacy Policy for Siddham Wellness." />
      </Head>
      <Navbar />
          ...
          <div style={{ maxWidth: 800, margin: '0 auto', background: 'var(--color-white)', padding: 'var(--space-8) var(--space-10)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--color-gray-100)' }}>
            {loading ? (
              ...
              <p style={{ color: 'var(--color-gray-400)', textAlign: 'center', fontStyle: 'italic' }}>
                Privacy policy details have not been set yet.
              </p>
            )}
          </div>
        </div>
