...
const STOCK_REPORT_CACHE_TTL = 60 * 2; // 2 minutes
...
export default async function handler(
  ...
  try {
    const cacheKey = "reports:stock:" + JSON.stringify(req.query || {});
    const fetchFresh = async () => {
      ...
      return { stockData, summary, csv };
    };

    const result = await getOrSet(cacheKey, STOCK_REPORT_CACHE_TTL, fetchFresh);
    res.setHeader(
      "Cache-Control",
