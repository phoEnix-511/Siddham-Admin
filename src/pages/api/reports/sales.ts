...
const SALES_REPORT_CACHE_TTL = 60 * 2; // 2 minutes
...
export default async function handler(
  ...
  try {
    const cacheKey = "reports:sales:" + JSON.stringify(req.query || {});
    const fetchFresh = async () => {
      ...
      return { totalRevenue, totalOrders, daily, productBreakdown, csv };
    };

    const result = await getOrSet(cacheKey, SALES_REPORT_CACHE_TTL, fetchFresh);
    res.setHeader(
      "Cache-Control",
